...
from strategies.shared.custom_indicators.rma import rma as shared_rma


def chop(candles: np.ndarray, length: int = 14, source_type="close", sequential=False) -> \
    ...
    # positives = np.where(positives < 0, 0, positives)
    # negatives = np.where(negatives > 0, 0, negatives)
    positives[positives < 0] = 0
    negatives[negatives > 0] = 0

    up = shared_rma(positives, length, sequential=True)
    down = shared_rma(-negatives, length, sequential=True)
    # print(down, len(down))
    rsi = rsi_fast(up, down)
    # print(rsi, len(rsi), len(down))
